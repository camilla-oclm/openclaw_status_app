...
{#if tl}<div class="section" id="trends" use:reveal><SectionHead title="Trends" note="tracked per assessment · adaptive cadence (8–48h)" id="trends" /><div class="trends-grid">{#each cards as card}<div class="tcard"><div class="tc-h"><span class="tc-t">{card.title}</span><span class="tc-s">{card.sub}</span></div>{#if card.spec}<Chart spec={card.spec} title={card.title} sub={card.sub} />{:else}<div class="tc-empty">Collecting — fills in as runs accumulate</div>{/if}</div>{/each}</div></div>{/if}
