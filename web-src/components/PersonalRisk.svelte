<script>
  ...
  import { keyVerdict, setupVerdict, shownVerdict, issuePlatforms, issueComponents, gateBlockers } from "../lib/verdict.js";
  ...
  // What the panel prints: a fresh ✅ reads "Too new to call", never a green "Safe to update"
  // over "we can't clear your setup yet" (fresh never softens, so rec === global there).
  const shown = $derived(shownVerdict(svd));
  const head = $derived.by(() => {
    if (svd.fresh) {
      const tail = "v" + (D.version || "") + " is a fresh release and version-specific reports are "
        + "still arriving, so we can't clear your setup yet. Back up before updating.";
      // The label already says "Too new to call" — lead with what IS known instead.
      if (shown.wait) return (hot.length ? "" : "Nothing blocking is reported for your stack so far — but ") + tail;
      return "Too new to call for your stack — " + tail;
    }
    if (svd.softened) {
