...
{#if vs.length}<div class="section" id="track-record" use:reveal><SectionHead title="Verdict track record" note="did the first read hold?" id="track-record" />{#if multi >= 1}<div class="tr-sum">{"Across " + multi + " version" + (multi === 1 ? "" : "s") + " with repeat assessments: " + bits.join(" · ") + "."}</div>{/if}<div class="tr-list">{#each vs as e}{@const b = badge(e)}<div class="tr-row{e.current ? ' cur' : ''}"><span class="tr-ver">v{e.version}{#if e.current}<span class="cur-dot" title="the current release">●</span>{/if}</span><span class="tr-path" title="Verdict sequence across this version's assessments">{#each e.path || [] as pv, idx}{#if idx}<span class="tr-arrow" aria-hidden="true">→</span>{/if}<span class="vgw {toneOf(pv)}"><Mark rec={pv} size={14} /></span>{/each}</span><span class="tr-runs">{e.runs + (e.runs === 1 ? " run" : " runs")}</span><span class="tr-badge{b.cls}" title={b.tip}>{b.txt}</span><span class="tr-dates">{dates(e)}</span></div>{/each}</div><div class="tr-note">Why this matters: a released version is immutable, so its issue list only accrues — an honest verdict should hold or harden between runs. This table is the receipts for the flip-conditions above: when one fires, the move shows up here.</div></div>{/if}
