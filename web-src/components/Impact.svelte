<script>
  ...
  import { COMP } from "../lib/tables.js";
  import { componentIssues, keyVerdict, shownVerdict } from "../lib/verdict.js";
...
{#if comp || plat}<div class="section" id="impact" use:reveal><SectionHead title="Impact" note="bar = issue volume · colour = worst severity" id="impact" />{#if entries.length}<div class="impact-sub" id="verdict-line"><div class="impact-sublabel">Verdict by component</div><div class="vline" role="list" aria-label="Per-component verdict" use:stagger>{#each entries as e}<span class="vchip{e.kv.blockers.length ? ' hot' : ''}{e.shown.wait ? ' wait' : ''}" role="listitem" aria-label="{e.name}: {lab(e)}, {e.n} issue{e.n === 1 ? '' : 's'}" title="{e.name}: {lab(e)} — {why(e)} Derived on this page from the issue tags; the official verdict stays global."><span class="vc-em" aria-hidden="true"><Mark rec={e.shown.rec} /></span><Icon k={e.ic} size={12} /><span class="vc-name">{e.name}</span><span class="vc-w" aria-hidden="true">{lab(e)}</span><span class="vc-n">{String(e.n)}</span></span>{/each}</div>{#if clear > 0}<div class="vline-note">{clear} other component{clear === 1 ? " has" : "s have"} no tracked issues this release.</div>{/if}</div>{/if}{#if comp}<div class="impact-sub" id="components"><div class="impact-sublabel">By component</div><div class="plat-grid" use:stagger>{#each comp as r (r.key)}{@render meter(r)}{/each}</div></div>{/if}{#if plat}<div class="impact-sub" id="platforms"><div class="impact-sublabel">By platform &amp; channel</div>{#if plat.flat}<div class="impact-note">Affects all platforms about equally — the meaningful split is by component, above.</div>{/if}<div class="plat-grid" use:stagger>{#each plat.rows as r (r.key)}{@render meter(r)}{/each}</div></div>{/if}</div>{/if}
